...
//   TCanvas * c3 = new TCanvas ("c3", "c3", 1200, 700);
  ...
  for (int ibin = 4; ibin <=24 ; ibin ++){
    ...
    if (DEBUG) cout << __LINE__ << endl;
    hSliceUpsilon.push_back( h1->ProjectionY(hName.str().c_str(), ibin, ibin, ""));
//     hSliceUpsilon.push_back( h1->ProfileY(hName.str().c_str(), ibin, ibin, ""));
    ...
    cout << __LINE__ <<  " " << h1->Integral() <<  " " << hSliceUpsilon[ihisto] ->Integral () << " " << entot << endl;
    
    ihisto++;

  }
  c3->Update();
  c3->SaveAs("AllFit_PtHigherMu.png");
...
//   TCanvas* c2= new TCanvas("c2", "c2", 1200,700) ;
  ...
  c2->SaveAs("FitResults_PtHigherMu.png");
