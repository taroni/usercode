...
//   c->cd(1);
  ...
  int entot = 0 ;
  vector <double> yield, mean, sigma, background, snratio;
  for (int ibin = 1; ibin <=35 ; ibin ++){
    ...
    yield.push_back(fittu->GetParameter(0));
    mean.push_back(fittu->GetParameter(1));
    sigma.push_back(fittu->GetParameter(2));
    background.push_back(bkg);
    snratio.push_back(fittu->GetParameter(0) / (fittu->GetParameter(0) + bkg));
  ...
  c3->Update();
  c3->SaveAs("AllFit_PtUpsilon.png");
  ...
  c2->cd(1); 
  hYield->Draw(); 
  hYield->GetXaxis()->SetTitle("p_{T}(#Upsilon) (GeV)");
  c2->cd(2);
  hMean->Draw();
  hMean->GetXaxis()->SetTitle("p_{T}(#Upsilon) (GeV)");
  hMean->GetYaxis()->SetRangeUser(9.3,9.6);
  c2->cd(3);
  hSigma->Draw(); 
  hSigma->GetXaxis()->SetTitle("p_{T}(#Upsilon) (GeV)");
  c2->cd(4);
  hBackground ->Draw();
  hBackground ->GetXaxis()->SetTitle("p_{T}(#Upsilon) (GeV)");
  c2->cd(5);
  hSnratio->Draw(); 
  hSnratio->GetXaxis()->SetTitle("p_{T}(#Upsilon) (GeV)");

  c2->SaveAs("FitResults_PtUpsilon.png");
