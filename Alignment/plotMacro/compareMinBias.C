...
using namespace std;
void compareMinBias( bool saveFig= 0){
  gROOT->Reset();
...
//   TFile * f = new TFile("");
//   TFile * f1 = new TFile("");
//   f = TFile::Open("outfileMinBias.root");
//   f1 = TFile::Open("outfileMinBias_newNOT.root");
//   if (!f){
...
//   }
  ...
  file = TFile::Open("rfio:/castor/cern.ch/cms/store/caf/user/taroni/testNewMinBiasNOTHLTSel/histoMinBias.root");
  file1 = TFile::Open("rfio:/castor/cern.ch/cms/store/caf/user/taroni/testNewMinBiasNOTHLTSel/histoMinBias_newNOT.root");
  if (!file){
    cout << "FILE NOT FOUND" << endl;
    return;
  }
  ...
  TFile* outfile = new TFile("compareMinBias.root","RECREATE");
  
  TDirectory * f = (TDirectory *) file -> Get ("myanalysis");
  TDirectory * f1 = (TDirectory *) file1 -> Get ("myanalysis");
...
//   h2Mass->Scale(1/h2Mass->Integral());
  ...
  h2Ntrk->Scale(1/h2Ntrk->Integral());
...
//   h2DeltaR->Scale(1/h2DeltaR->Integral());
  ...
  c1->cd(2);
  hNtrk->Draw();
  hNtrk->GetXaxis()->SetRangeUser(0,500);
  hNtrk->SetFillColor(0);
  hNtrk->SetLineColor(1);
  hNtrk->SetLineWidth(2);
  h2Ntrk->Draw("SAMES");
  h2Ntrk->SetFillColor(0);
  h2Ntrk->SetLineColor(2);
  h2Ntrk->SetLineWidth(2);
  c1->cd(3);
  c1->GetPad(3)->SetLogy();
  hP->Draw();
  hP->GetXaxis()->SetRangeUser(0,100);
  hP->SetFillColor(0);
  hP->SetLineColor(1);
  hP->SetLineWidth(2);
  h2P->Draw("SAMES");
  ...
  c3->cd(3);
  h2d0vspt->Draw();
  h2d0vspt->GetXaxis()->SetRangeUser(0.,100.);
