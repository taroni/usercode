...
// vector < Profile *>  hSlice, hSliceUpsilon; 
vector < TH1D *>  hSlice, hSliceUpsilon; 

using namespace std;
void fitMassvsLowMuonPt(){
  gROOT->Reset();
...
//   TCanvas * c3 = new TCanvas ("c3", "c3", 1200, 700);
  ...
  for (int ibin = 2; ibin <=20 ; ibin ++){
    ...
    if (DEBUG) cout << __LINE__ << endl;
    hSliceUpsilon.push_back( h2->ProjectionY(hName.str().c_str(), ibin, ibin, ""));
//     hSliceUpsilon.push_back( h1->ProfileY(hName.str().c_str(), ibin, ibin, ""));
    if (DEBUG) cout << __LINE__ << endl;
    c3->cd(ihisto+1);
    if (DEBUG) cout << __LINE__ << endl;
    ...
    cout << __LINE__ <<  " " << h2->Integral() <<  " " << hSliceUpsilon[ihisto] ->Integral () << " " << entot << endl;
    
    ihisto++;

  }
  c3->Update();
  c3->SaveAs("AllFit_PtLowMu.png");
...
//   TCanvas* c2= new TCanvas("c2", "c2", 1200,700) ;
  ...
  c2->SaveAs("FitResults_PtLowMu.png");
