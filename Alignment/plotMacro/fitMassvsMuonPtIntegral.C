...
//   TCanvas * c3 = new TCanvas ("c3", "c3", 1200, 700);
  ...
  for (int ibin = 4; ibin <=24 ; ibin ++){
    ...
    if (DEBUG) cout << __LINE__ << endl;
    hSliceUpsilon.push_back( h1->ProjectionY(hName.str().c_str(), ibin, h3->GetYaxis()->GetLast(), ""));
//     hSliceUpsilon.push_back( h1->ProfileY(hName.str().c_str(), ibin, h3->GetYaxis()->GetLast(), ""));
  ...
  c3->Update();
  c3->SaveAs("AllFit_PtHigherMuIntegral.png");
...
//   TCanvas* c2= new TCanvas("c2", "c2", 1200,700) ;
  ...
  c2->SaveAs("FitResults_PtHigherMuIntegral.png");
