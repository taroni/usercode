...
//   TFile * f = new TFile("");
//   TFile * f1 = new TFile("");  
//   f = TFile::Open("outfileMuIso.root");
//   f1 = TFile::Open("outfileMuIso_newHLT.root");
//   if (!f){
...
//   }
  ...
  file = TFile::Open("rfio:/castor/cern.ch/cms/store/caf/user/taroni/testNewMinBiasNOTHLTSel/histoMuIso.root");
  file1 = TFile::Open("rfio:/castor/cern.ch/cms/store/caf/user/taroni/testNewMinBiasNOTHLTSel/histoMuIso_newHLT.root");
  TFile* outfile = new TFile("compareMuIso.root","RECREATE");
  if (!file){
    cout << "FILE NOT FOUND" << endl;
    return;
  }
...
//   h2DeltaR->Scale(1/h2DeltaR->Integral());
  ...
  c1->GetPad(2)->SetLogy();
  hNtrk->Draw();
  hNtrk->GetXaxis()->SetRangeUser(0,20);
