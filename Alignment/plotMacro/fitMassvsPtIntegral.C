...
//   outfile -> cd(); 
  ...
  for (int ibin = 1; ibin <=35 ; ibin ++){
    ...
    if (ibin ==4 ) fittu->SetParameter(3,1000.);
    ...
    if (DEBUG) cout << __LINE__ << endl;
    hSliceUpsilon.push_back( h3->ProjectionY(hName.str().c_str(), ibin, h3->GetYaxis()->GetLast(), ""));
//     hSliceUpsilon.push_back( h3->ProfileY(hName.str().c_str(), ibin, h3->GetYaxis()->GetLast(), ""));
    if (DEBUG) cout << __LINE__ << endl;
    if (ibin==1) {
      c1->cd();
  ...
  c3->Update();
  c3->SaveAs("AllFit_PtUpsilonIntegral.png");
  ...
  c2->SaveAs("FitResults_PtUpsilonIntegral.png");
