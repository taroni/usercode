...
class GenSystReco : public edm::one::EDAnalyzer<edm::one::WatchRuns>  {
   public:
      explicit GenSystReco(const edm::ParameterSet&);
      ~GenSystReco();
  ...
  //      edm::EDGetTokenT<reco::GenParticleCollection> tok_genPart_;
      ...
      edm::EDGetTokenT<pat::MuonCollection> tok_recoMuons_;
      edm::EDGetTokenT<pat::ElectronCollection> tok_recoElectrons_;
      edm::EDGetTokenT<pat::TauCollection> tok_recoTaus_;
...
//
GenSystReco::GenSystReco(const edm::ParameterSet& iConfig)

{
   ...
   //tok_recoPart_ =consumes<pat::PackedCandidateCollection>(edm::InputTag("packedCandidates"));
   tok_recoMuons_ = consumes<pat::MuonCollection>(edm::InputTag("slimmedMuons"));
   tok_recoElectrons_ = consumes<pat::ElectronCollection>(edm::InputTag("slimmedElectrons"));
   tok_recoTaus_ = consumes<pat::TauCollection>(edm::InputTag("slimmedTaus"));
   ...
   tok_vtx_ =consumes<reco::VertexCollection>(edm::InputTag("offlineSlimmedPrimaryVertices")); 
   tok_bs_ =consumes<reco::BeamSpot>(edm::InputTag("offlineBeamSpot")); 

}


GenSystReco::~GenSystReco()
{
...
void
GenSystReco::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  nevent_run++;
   using namespace edm;
  ...
  iEvent.getByToken(tok_vtx_, primaryVertex);
  const reco::Vertex* pv = &*primaryVertex->begin();
  
  edm::Handle<pat::MuonCollection> recoMuons; 
  iEvent.getByToken(tok_recoMuons_,recoMuons);
  edm::Handle<pat::ElectronCollection> recoElectrons; 
  iEvent.getByToken(tok_recoElectrons_,recoElectrons);

  edm::Handle<pat::TauCollection> recoTaus; 
  iEvent.getByToken(tok_recoTaus_,recoTaus);
  ...
  vector<const pat::Muon *> mus ;
  vector<const pat::Tau *> taus;
  vector<const pat::Electron *> etaus;

  for (pat::MuonCollection::const_iterator p=recoMuons->begin(); p!=recoMuons->end(); ++p){
    bool fromHiggs= false;
    if (p->genParticle(0)!=0){
      const reco::Candidate *mother = 0;
      if( p->genParticle(0)->numberOfMothers()!=0) mother= p->genParticle(0);
      for (unsigned int im =0; im<10 ; im++){
	if (mother->numberOfMothers()!=0 ) {
	  mother=&*mother->mother(0);
	  if (mother->pdgId()==25){
	    fromHiggs=true;
	    //cout << __LINE__ << " gen particle " << p->genParticle(0)->pdgId()<<" " <<  mother->pdgId() << endl; 
	    break;
	  }
	}
      }
    }
    if (fromHiggs!=true ) continue;
    if (p->isGlobalMuon()==false) continue;
    if (p->pt() < 5) continue;
    if (abs(p->eta())> 2.1) continue;
    if (!p->isTightMuon(*pv) ) continue; 
    if (p->bestTrack()->dxy(beamSpot)>0.2) continue;
    double relIso = (p->userIsolation("PfChargedHadronIso")+max(p->userIsolation("PfNeutralHadronIso") +p->userIsolation("PfGammaIso") -0.5*p->userIsolation("PfPUChargedHadronIso"),0.0)) /p->pt();
  ...
  for (pat::ElectronCollection::const_iterator p=recoElectrons->begin(); p!=recoElectrons->end(); ++p){
    bool fromHiggs= false;
    if (p->genParticle(0)!=0){
      if (abs(p->genParticle(0)->pdgId())!=11) continue;
      const reco::Candidate *mother = 0;
      if( p->genParticle(0)->numberOfMothers()!=0) mother= p->genParticle(0);
      for (unsigned int im =0; im<10 ; im++){
	if (mother->numberOfMothers()!=0 ) {
    ...
    if (fromHiggs!=true ) continue;
    if (p->isElectron() && p->pt()>10 && abs(p->eta())<2.3) {
      if (p->gsfTrack()->dxy(beamSpot)>0.2) continue;
      if (!p->passConversionVeto ()) continue;
      //if (!p->isElectronIdAvailable())continue;
  ...
  for (pat::TauCollection::const_iterator p=recoTaus->begin(); p!=recoTaus->end(); ++p){
    bool fromHiggs= false;
    if (p->genParticle(0)!=0){
      if (abs(p->genParticle(0)->pdgId())!=15) continue;
      const reco::Candidate *mother = 0;
      if( p->genParticle(0)->numberOfMothers()!=0) mother= p->genParticle(0);
      for (unsigned int im =0; im<10 ; im++){
	if (mother->numberOfMothers()!=0 ) {
  ...
  if (mus.size() != 1 || etaus.size()> 1 || taus.size()>1) return;
  ...
  for (pat::JetCollection::const_iterator jet = jets->begin(); jet!=jets->end(); jet++){
    ...
    if (taus.size()!=0){
      if ( reco::deltaR(jet->eta(),jet->phi(),taus[0]->eta(),taus[0]->phi()) < 0.4) continue;
    }
    if(mus.size()!=0) {
      if ( reco::deltaR(jet->eta(),jet->phi(),mus[0]->eta(),mus[0]->phi()) < 0.4) continue;
    }
...
if (njet > 2) return; 
  ...
  const pat::Muon * mu=0;
  const pat::Electron* etau=0;
  const pat::Tau* tau=0;

  mu=&*mus[0];
  if (etaus.size()==1) etau=&*etaus[0];
  if (taus.size()==1)tau=&*taus[0];
  
  bool hadDecay=false;
  if (etau==0 && tau!=0){
    hadDecay=true;
  } else if (etau==0 && tau==0 ){
    return;
  } else if (etau!=0 && tau!=0 ) {
    //cout << "Both etau and tau found" << endl;
  }
...
if (DEBUG) cout << __LINE__ << endl;
  ...
  if (mu->pt()<25 ) return;
  double relIsoMu = (mu->userIsolation("PfChargedHadronIso")+max(mu->userIsolation("PfNeutralHadronIso") +mu->userIsolation("PfGammaIso") -0.5*mu->userIsolation("PfPUChargedHadronIso"),0.0)) /mu->pt();
  if(relIsoMu> 0.15) return; 
  ...
  if (hadDecay) {
    ...
    if (!tau->isTauIDAvailable("decayModeFinding") ) return; 
    if (!tau->tauID("decayModeFinding") ) return; 
    if (!tau->tauID("againstElectronMediumMVA6") ) return;
    if (!tau->tauID("againstMuonTight3")) return;
    if (DEBUG) cout << __LINE__ << endl;
  
  } else{
    if (DEBUG) cout << __LINE__ << endl;
    if (abs(etau->eta())> 2.3 ) return;
    if (etau->pt()<10)return; 
    if (!etau->electronID("mvaEleID-Spring15-25ns-nonTrig-V1-wp90")) return;
    double relIso = (etau->userIsolation("PfChargedHadronIso")+max(etau->userIsolation("PfNeutralHadronIso") +etau->userIsolation("PfGammaIso") -0.5*etau->userIsolation("PfPUChargedHadronIso"),0.0)) /etau->pt();
...
void
GenSystReco::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
void
GenSystReco::beginRun( edm::Run const & iRun,  edm::EventSetup const& iSetup) {
  nevent_run = 0;
}
void
 GenSystReco::endRun( const edm::Run& iRun, const edm::EventSetup& iSetup) {
...
double GenSystReco::dPhi(math::XYZTLorentzVector& vec1, math::XYZTLorentzVector& vec2) {
  return reco::deltaPhi(vec1.phi(),vec2.phi());
}

//define this as a plug-in
DEFINE_FWK_MODULE(GenSystReco);
