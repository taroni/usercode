...
#include "SimpleAnalyzer/ZeeAnalyzer/plugins/ZeeSkim.h"
//
...
//
ZeeSkim::ZeeSkim(const edm::ParameterSet& iConfig):
...
bool
ZeeSkim::filter(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  nSelectedElectronsEB =0 ; 
  nSelectedElectronsEE =0 ; 
  using namespace edm;
  ...
  std::cout << __LINE__<< " filter result: "<< keepEvent << " nSelectedEvents "<< nSelectedEvents << " nSelectedElectrons "<< nSelectedElectronsEB << " + " << nSelectedElectronsEE << std::endl;
  return keepEvent;
}
void 
ZeeSkim::beginJob() {
