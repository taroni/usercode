...
void chargeProfileRatioNoBiasScan(string filename="lorentzangleALCARECO.root",  bool saveFig= 0){
  ...
  for (int ilayer = 1; ilayer<4; ilayer ++){
    for (unsigned int ibias = 0; ibias < vbias.size(); ibias++){
      vector <TH1D*> vHisto;
      name.str(""); 
      name << "h_ratiovsmodule_layer"<< ilayer<<"_bias" << vbias[ibias] ; 
	...
	hy->Divide(h2y); 
	name.str("");
	name<< "chargeProfile_layer"<< ilayer<< "_module"<< imodule<< "_bias_"<< vbias[ibias]; 
	...
	if (DEBUG) cout <<__LINE__ << endl;
	name.str("");
	name << "hratio_module"<< 9-imodule << "_module" << imodule << "_bias"<< vbias[ibias]; 
	if (DEBUG) cout <<__LINE__ << endl;
	...
	fprintf (pFile,"%i\t %i\t %i\t %f\t %f\n", ilayer, imodule, vbias[ibias], f->GetParameter(0), f->GetParError(0)); 
