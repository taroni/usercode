...
process.AODoutput = cms.OutputModule("PoolOutputModule",
    ...
    fileName = cms.untracked.string('GGHetau_8TeVBeamSpot_40PU_AODSIM_"""+str(1+i)+""".root'),
...
"""
    ...
    cfg_file = open("ggHiggsETau_8TeVBeamSpot_40PU-"+str(i)+"_cfg.py","w")
    cfg_file.write(cfg)
    cfg_file.close()
    ...
    sh = """#!/bin/tcsh -f
            ...
            set CFG =   \"/afs/cern.ch/user/t/taroni/scratch1/LFVH/CMSSW_7_2_3/src/FastSimulation/Event/test/ggHiggsETau_8TeVBeamSpot_40PU-"""+str(i)+"""_cfg.py\"
            cd $W_DIR
            eval `scramv1 runtime -csh`
            cd -
            cmsRun $CFG
            cmsStage GGHetau_8TeVBeamSpot_40PU_AODSIM_"""+str(1+i)+""".root /store/user/taroni/723/ggHiggsETau/
            
            exit
    """

    #scrive script
    sh_file = open("submitggHiggsETau_8TeVBeamSpot_40PU-"+str(i)+".sh","w")
    sh_file.write(sh)
    sh_file.close()

    #sottomette script
    popen("chmod a+x submitggHiggsETau_8TeVBeamSpot_40PU-"+str(i)+".sh" )
    popen("bsub -q 1nw submitggHiggsETau_8TeVBeamSpot_40PU-"+str(i)+".sh")
